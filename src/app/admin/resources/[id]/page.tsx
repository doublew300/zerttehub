    ...
    const handleSubmit = async (e: React.FormEvent) => {
        ...
        try {
            ...
            const { error } = await supabase
                .from('resources')
                .update({ ...formData, file_url: finalUrl })
                .eq('id', params.id)
                ...
                <div className="bg-white/5 border border-white/10 rounded-3xl p-8 backdrop-blur-xl shadow-2xl">
                    <form onSubmit={handleSubmit} className="space-y-6">
                        ...
                        <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
                            ...
                            {/* Downloads */}
                            <div>
                                <label className="block text-sm font-medium text-gray-400 mb-2">Скачиваний</label>
                                <input
                                    type="number"
                        ...
                        >
                            {saving || uploading ? <Loader2 className="animate-spin w-5 h-5" /> : <Save className="w-5 h-5" />}
                            {uploading ? 'Загрузка файла...' : 'Сохранить изменения'}
                        </button>
